package com.raphydaphy.thaumcraft.render;
...
public class RenderEntityItemFancy extends Render<EntityItemFancy> {
    ...
    public static void renderFancyBeams(double x, double y, double z, Color effectColor, long seed, long continuousTick, int dstJump, float scale, int countFancy, int countNormal) {
        ...
        for (int i = 0; i < fancy_count; i++) {
            ...
            vb.pos(0, 0, 0).color(effectColor.getRed(), effectColor.getGreen(), effectColor.getBlue(), (int) (255.0F * (1.0F - beamSize))).endVertex();
