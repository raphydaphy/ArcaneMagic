...
public class CategoryRegistry implements IForgeRegistry<NotebookCategory> {
	...
	public void register(NotebookCategory value) {
		...
		if (oldEntry == value) {
			ArcaneMagic.LOGGER.error("Category Registry: The object {} has been registered twice for the same name {}.", value, key);
			return;
	...
	private static void freeze() {
		ArcaneMagic.LOGGER.info("Freezing Category Registry - being called from " + Thread.currentThread().getStackTrace()[0]);
		frozen = true;
	}
	
	public static void sortCategories() {
		List<NotebookCategory> ours = NotebookCategory.REGISTRY.getValues().stream().filter((c) -> c.getRegistryName().getResourceDomain().equals(ArcaneMagic.MODID)).collect(Collectors.toList());
		List<ResourceLocation> theirNames = new ArrayList<>();
		NotebookCategory.REGISTRY.getValues().stream().filter((c) -> !c.getRegistryName().getResourceDomain().equals(ArcaneMagic.MODID)).sorted(Collator.getInstance()).forEach((c) -> theirNames.add(c.getRegistryName()));
