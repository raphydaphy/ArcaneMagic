...
public class ScepterRegistry {
	...
	public static void registerDefaults() {
		if(!isDone)
		registerAll(ScepterCap.IRON, ScepterCap.GOLD, ScepterCap.THAUMIUM, ScepterCore.WOOD, ScepterCore.GREATWOOD, ScepterCore.SILVERWOOD);
		isDone = true;
	}
	...
	public static boolean register(ScepterPart part) {
		...
		if (oldEntry == part) {
			ArcaneMagic.LOGGER.error("Scepter Registry: The object {} has been registered twice for the same name {}.", part, key);
			return false;
		} else if (oldEntry != null) {
			ArcaneMagic.LOGGER.error("Attempting to register {} in the Scepter Registry, but it already existed! Old part: {}, using name {}", part, oldEntry, key);
			return false;
		}
	...
	public static void registerAll(ScepterPart... parts) {
		for(ScepterPart p : parts) register(p);
	}
