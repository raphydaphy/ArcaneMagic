...
public class ModEvents {
	...
	public static void onLivingEquipmentChange(LivingEquipmentChangeEvent event) {
		if (!event.getEntityLiving().world.isRemote) {
			...
			// decrease reach
			if (event.getTo().getItem().equals(ModRegistry.ARCANE_DAGGER) && !event.getFrom().getItem().equals(ModRegistry.ARCANE_DAGGER)) {
				event.getEntityLiving().getEntityAttribute(EntityPlayer.REACH_DISTANCE).setBaseValue(3);
			}

			// reset reach
			else if (event.getFrom().getItem().equals(ModRegistry.ARCANE_DAGGER) && !event.getTo().getItem().equals(ModRegistry.ARCANE_DAGGER)) {
