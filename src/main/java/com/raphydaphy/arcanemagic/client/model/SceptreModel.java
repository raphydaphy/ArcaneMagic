...
public class SceptreModel implements IModel
{
	...
	static
	{
		try
		{
			...
			bakeImpl = ReflectionHelper.findMethod(VanillaModelWrapper, "bakeImpl", null, IModelState.class,
					VertexFormat.class, Function.class);
		} catch (Exception e)
		{
	 ...
	 */
	private static IBakedModel bakeImpl(IModel model, IModelState state, VertexFormat format,
	...
	public class BakedSceptre implements IBakedModel
	{
		...
		public BakedSceptre(VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter)
		{
			...
			if (SceptreModel.this.textured)
			{
				...
				this.bakedBase = bakeImpl(SceptreModel.this.sceptreBase,
						SceptreModel.this.sceptreBase.getDefaultState(), format, bakedTextureGetter);
			}
		}
		...
		{
			return this.bakedBase != null ? this.bakedBase.getParticleTexture()
					: Minecraft.getMinecraft().getTextureMapBlocks().getTextureExtry("missingno");
		}
			...
			return new ItemOverrideList(ImmutableList.of())
			{
						...
						@Nullable EntityLivingBase entity)
				{
					if (stack.getItem() == ModRegistry.SCEPTER)
					{
						...
						IBakedModel generated = SceptreModel.this.retexture(tex.build()).bake(
								SceptreModel.this.getDefaultState(), BakedSceptre.this.format,
								BakedSceptre.this.bakedTextureGetter);
