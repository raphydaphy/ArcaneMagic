...
public class GuiThaumonomicon extends GuiScreen
{
	...
	private static final ResourceLocation frame = new ResourceLocation(Thaumcraft.MODID,
			"textures/gui/thaumonomicon.png");
	private static final ResourceLocation page = new ResourceLocation(Thaumcraft.MODID,
			"textures/gui/thaumonomicon_page.png");
	private static final ResourceLocation back = new ResourceLocation(Thaumcraft.MODID,
			"textures/gui/thaumonomicon_back.png");
	private static final ResourceLocation back_eldritch = new ResourceLocation(Thaumcraft.MODID,
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks)
	{
		...
		{
			int thisTab = tab >= ThaumcraftAPI.TOTAL_THAUMONOMICON_CATEGORIES
					? tab - ThaumcraftAPI.TOTAL_THAUMONOMICON_CATEGORIES
					: tab;
		...
		for (IThaumonomiconEntry entry : ThaumcraftAPI.THAUMONOMICON_CATEGORIES
		...
		if (relMouseX >= screenX - 24 && relMouseY >= screenY && relMouseX <= screenX
				&& relMouseY <= screenY + (ThaumcraftAPI.TOTAL_THAUMONOMICON_CATEGORIES * 23))
		{
			for (int tab = 0; tab < ThaumcraftAPI.TOTAL_THAUMONOMICON_CATEGORIES; tab++)
			{
						...
						&& relMouseX >= screenX - tabWidth)
				{
					this.fontRenderer.drawString(
							I18n.format(ThaumcraftAPI.THAUMONOMICON_CATEGORIES.get(tab).getUnlocalizedName()),
					...
					if (relMouseY >= screenY + (tab * 23) && relMouseY <= screenY + (tab * 23) + 23)
					{
						if (player.getEntityData().getInteger(tagTab) != tab)
						{
							player.getEntityWorld().playSound(player.posX, player.posY, player.posZ,
									ThaumcraftSoundHandler.randomCameraClackSound(), SoundCategory.MASTER, 1f, 1f,
									false);
